...
-- Abfrage
SELECT * FROM [dbo].[vwView]; -- Es sind 3 Spalten

-- Spalte hinzufügen
ALTER TABLE dbo.Tickets ADD Username varchar(50) NULL;

SELECT * FROM [dbo].[vwView]; -- Es sind immer noch 3 Spalten

-- Nun ALTER VIEW mit gleicher Definition
GO
...
GO
-- ODER
EXEC sp_refreshview '[dbo].[vwView]';


SELECT * FROM [dbo].[vwView]; -- Nun sind es wie erwartet 4 Spalten

-- Zugriff aus temporäre Tabelle möglich? Nein!
SELECT * INTO #t1 FROM [dbo].[Tickets];
SELECT * FROM #t1;

-- View anlegen
CREATE VIEW [dbo].[vwView2]
AS
	SELECT * FROM #t1;
GO
CREATE TABLE [Sales].[SalesOrderHeader] (
    ...
    CONSTRAINT [FK_SalesOrderHeader_SalesPerson_SalesPersonID] FOREIGN KEY ([SalesPersonID]) REFERENCES [Sales].[Verkaeufer] ([BusinessEntityID]),
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            UPDATE [Sales].[Verkaeufer]
            SET [Sales].[Verkaeufer].[SalesYTD] = 
                (SELECT SUM([Sales].[SalesOrderHeader].[SubTotal])
                FROM [Sales].[SalesOrderHeader] 
                WHERE [Sales].[Verkaeufer].[BusinessEntityID] = [Sales].[SalesOrderHeader].[SalesPersonID]
                    AND ([Sales].[SalesOrderHeader].[Status] = 5) -- Shipped
                    AND [Sales].[SalesOrderHeader].[OrderDate] BETWEEN @StartDate AND @EndDate)
            WHERE [Sales].[Verkaeufer].[BusinessEntityID] 

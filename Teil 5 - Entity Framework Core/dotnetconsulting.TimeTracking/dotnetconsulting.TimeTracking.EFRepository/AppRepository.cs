...
using AutoMapper;
using dotnetconsulting.TimeTracking.EFRepository.Automapper;

namespace dotnetconsulting.TimeTracking.EFRepository;
...
public class AppRepository : IAppRepository
{
    ...
    public AppRepository(TimeTrackingContext Context)
    {
        _context = Context;
    ...
    {
        // Daten abfragen
        var query = _context.Orders!.Include(p => p.Customer);
    ...
    {
        // Daten abfragen
        var query = _context.Entries!.Include(p => p.Order.Customer)
            .Where(w => (Start == null || w.Day >= Start.Value) &&
                        (End == null || w.Day <= End.Value) &&
                        (OrderId == null || w.Order.Id == OrderId.Value));

        // Ergebnis abrufen
        IList<Entry> rawResult = await query.ToListAsync(cancellationToken).ConfigureAwait(false);
        cancellationToken.ThrowIfCancellationRequested();
    ...
    public async Task StoreTrackingEntryAsync(EntryDto Entry, CancellationToken cancellationToken)
    {
        ...
        if (rawResult == null)
        {
            ...
            rawResult = _context.Entries.Add(new Entry()).Entity;
        }

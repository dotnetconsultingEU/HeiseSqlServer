...
SELECT DENSE_RANK() OVER (ORDER BY [Umsatz]) AS 'Gesamtrang',
	   * 
...
SELECT DENSE_RANK() OVER (ORDER BY [Umsatz]) AS 'Gesamtrang',
	   DENSE_RANK() OVER (PARTITION BY [Land] ORDER BY [Umsatz]) AS 'Landesrang',
	   * 
FROM [dbo].[Vertriebszahlen]
ORDER BY [Name];
...
-- SQL Server 2022, Umsatz auf je 1000 (abgerundet)
-- CTE verwenden, um den Ausdruck nur einmal im Code zu haben
;WITH [cte]
AS
(
...
SELECT DENSE_RANK() OVER win1 AS 'Gesamtrang',
	   DENSE_RANK() OVER win2 AS 'Landesrang',
	   FORMAT([Umsatz], 'N0'),
	   [Name],
	   [Land]
FROM [cte]
